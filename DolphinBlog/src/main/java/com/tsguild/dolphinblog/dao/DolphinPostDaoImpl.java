...
//I edited this to only load in posts that have yet to be approved by the admin.
    //I edited this to only load in posts that have yet to be approved by the admin.
    //still need Hashtags (see below)
//            SELECT Posts.title, hashtag_name FROM Posts LEFT JOIN HashtagsPosts 
//            ON Posts.post_id= HashtagsPosts.post_id
//            LEFT JOIN Hashtags
//            ON HashtagsPosts.hashtag_id=Hashtags.hashtag_id; (or you can add WHERE post_id = ?) for get by ID
    ...
    String SQL_GET_FRONT_PAGE_IDS="SELECT Posts.post_id, Posts.status_id FROM Posts Where status_id = 3 Order by Posts.post_id DESC";
    @Override
    public List<Integer> getAllActivePostsID() {
        return jdbcTemplate.query(SQL_GET_FRONT_PAGE_IDS, new activePostMapper());
    }
...
//    }
    ...
    private static final class activePostMapper implements RowMapper<Integer> {
